...
public class SQLQueries {
    ...
    public static class PATSQLQueries {
        ...
        public static final String GET_TOKENS_METADATA = "SELECT TOKEN_ID, ALIAS, DESCRIPTION, VALIDITY_PERIOD, " +
                "TIME_CREATED FROM IDN_OAUTH2_ACCESS_TOKEN NATURAL JOIN IDN_PAT_DATA WHERE SUBJECT_IDENTIFIER = ? " +
                "AND GRANT_TYPE = ?  ORDER BY TOKEN_ID";

        public static final String GET_CLIENT_ID_FROM_TOKEN_ID = "SELECT INBOUND_AUTH_KEY FROM " +
                "IDN_OAUTH2_ACCESS_TOKEN JOIN SP_INBOUND_AUTH " +
                "WHERE CONSUMER_KEY_ID = APP_ID AND TOKEN_ID = ?";
